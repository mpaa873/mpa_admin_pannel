    ...
    <div className="max-w-7xl mx-auto">
        ...
        {STATS_DATA.map((stat, idx) => (
          <div key={idx} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="flex items-baseline justify-between">
              <h3 className={`text-3xl font-bold ${stat.color}`}>{stat.value.toLocaleString()}</h3>
              <span className="text-[10px] bg-slate-100 px-2 py-1 rounded-full text-slate-600 uppercase font-bold tracking-wider">Live</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-50 flex justify-between items-center">
          <h2 className="text-xl font-bold text-slate-800">Pending Actions & Submissions</h2>
            ...
            <tbody className="divide-y divide-slate-100">
              {RECENT_SUBMISSIONS.map((paper) => (
                <tr key={paper.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-slate-500 text-sm">{paper.date}</td>
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full text-[11px] font-bold uppercase ${getStatusStyles(paper.status)}`}>
                      {paper.status}
                    </span>
                  </td>
...
function getStatusStyles(status) {
  switch (status.toLowerCase()) {
    ...
    case 'minor revision': return 'bg-amber-100 text-amber-700';
    case 'proofreading': return 'bg-purple-100 text-purple-700';
